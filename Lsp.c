...
void Sorting(Word* head,Word* array[],int count,int read) {
	...
	int main(){
		...
		char* number = malloc(sizeof(char)*20); // uesr로부터 입력받을 파일 번호를 문자열로 저장.
		int mode = 0; // 기능변수
		...
		while(1) {
			...
			Sorting(head,array,count,read); // Word구조체 배열에 정렬 혹은 랜덤으로 단어가 정리되서 튀어나옴. 이제 이걸 각 기능별로 보내서 구현
